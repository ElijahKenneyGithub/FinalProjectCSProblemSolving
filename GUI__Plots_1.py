...
class View:
    ...
    def adjust_frequency(self, new_target_frequency):
        # Function to adjust the target frequency to different values based on button clicks
        # Update the target frequency and perform other necessary actions here
        self.target_frequency = new_target_frequency
        print(f"Target Frequency set to: {self.target_frequency}")

    def create_frequency_buttons(self):
        # Create buttons for adjusting target frequency to different values

        low_button = tk.Button(self.root, text="Low Frequency (5000)", command=lambda: self.adjust_frequency(5000))
        low_button.pack()

        medium_button = tk.Button(self.root, text="Medium Frequency (10000)", command=lambda: self.adjust_frequency(10000))
        medium_button.pack()

        high_button = tk.Button(self.root, text="High Frequency (15000)", command=lambda: self.adjust_frequency(15000))
        high_button.pack()



    def combine_plots(self):
         # Check if the necessary data is available to combine plots
        if self.data is not None and self.spectrum is not None and self.freqs is not None and self.t is not None and self.data_in_db is not None:
            # Combine and plot the data here as required
            # Example: Combine plots using matplotlib to display in a separate window
            fig_combined, ax_combined = plt.subplots(1, 1, figsize=(10, 6))

            # Plotting the combined data
            # For example, you can plot self.t versus self.data_in_db
            ax_combined.plot(self.t, self.data_in_db, label='Combined Plot')

            # Set labels, title, etc. for the combined plot
            ax_combined.set_xlabel('Time (s)')
            ax_combined.set_ylabel('Power (dB)')
            ax_combined.set_title('Combined Plot')
            ax_combined.legend()

            # Show the combined plot in a new window
            new_window_combined = tk.Toplevel(self.root)
            frame_combined = tk.Frame(new_window_combined)
            frame_combined.pack()

            canvas_combined = FigureCanvasTkAgg(fig_combined, master=frame_combined)
            canvas_combined.get_tk_widget().pack(expand=1, padx=10, pady=10)
        pass
    ...
    def display_plots(self):
        ...
        def find_target_frequency(freqs):
            for x in freqs:
                print(x)
                if x > self.target_frequency:  # This can be changed to find a specific range, standard is 1000
                    break
            return x
